...
interface Props {
  ...
  subtitle?: string;
  context?: string; // page/segment context, sent as `segment`
...
const GlobalCTA = ({ title, subtitle, context, defaultService }: Props) => {
  const { settings } = useSiteSettings();
  const [data, setData] = useState({
    name: '', company: '', whatsapp: '', email: '', service: defaultService || '', message: '',
  });
  const { submit, isSubmitting, isSubmitted, errors } = useContactForm({
    onSuccess: () => setData({ name: '', company: '', whatsapp: '', email: '', service: defaultService || '', message: '' }),
  });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await submit({
      name: data.name, email: data.email, phone: data.whatsapp,
      company: data.company, service: data.service,
      segment: context, message: data.message,
    });
  };

  const waMessage = encodeURIComponent(
    `Olá! Sou ${data.name || '[nome]'}${data.company ? ` da ${data.company}` : ''}.${context ? ` Vim pela página ${context}.` : ''} Tenho interesse em ${data.service || 'conversar'}. ${data.message}`
  );
  const waLink = `https://wa.me/${settings.whatsapp}?text=${waMessage}`;

  return (
    <section className="section-padding relative overflow-hidden">
      <SiteBackdrop section="cta" intensity={25} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-14 items-start">
          <div className="lg:col-span-2">
            ...
            <h2 className="text-3xl md:text-5xl font-display font-bold mb-6 leading-tight">
              {title || (
                <>Vamos conversar sobre o seu <span className="text-gradient-neon italic">projeto?</span></>
              )}
            </h2>
            <p className="text-muted-foreground text-base md:text-lg mb-8">
              {subtitle || 'Conte para a Racun o momento da sua marca. Um especialista entra em contato com um plano estratégico sob medida para o seu próximo passo.'}
            </p>
            <a
              href={waLink}
              target="_blank"
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input name="name" value={data.name} onChange={onChange} required placeholder="Seu nome completo"
                  ...
                  <div>
                    ...
                    <input name="company" value={data.company} onChange={onChange} placeholder="Nome da sua empresa"
                      className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/40 hover:border-white/20 transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium uppercase tracking-wider mb-2 text-muted-foreground">WhatsApp</label>
                    <input name="whatsapp" value={data.whatsapp} onChange={onChange} type="tel" placeholder="(00) 00000 0000"
                      className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/40 hover:border-white/20 transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium uppercase tracking-wider mb-2 text-muted-foreground">E-mail *</label>
                    <input name="email" value={data.email} onChange={onChange} type="email" required placeholder="seu@email.com"
                      className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/40 hover:border-white/20 transition-all" />
                ...
                <div>
                  ...
                  <select name="service" value={data.service} onChange={onChange} required
                ...
                <div>
                  ...
                  <textarea name="message" value={data.message} onChange={onChange} required rows={4}
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  ...
                  <a href={waLink} target="_blank" rel="noopener noreferrer" className="btn-outline flex items-center justify-center gap-2">
